...
* the resulting compression is not as good as H264 (not available for writing since it is licenced) and we end up with files that doubled in size.

### FFMPEG
**FFMPEG** is available in WSL so we can just call it to concatenate files
```
ffmpeg -f concat -safe 0 -i rev_listtoconcat.txt -c copy ~/c/Temp/todel/big.mp4
```
It is fast and the resulting file is H264 so the size is not altered much.
...
```
One will noticed that I mounted **WDMyCLoud** to wsl.  
Here is the command to do it (it only works because the folder was already shared in windows):
```
